...
export class WalletTransactionController {
  ...
  async getUserWalletTransactions(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const result = await this.walletTransactionService.getUserWalletTransactions(
        userId,
        ...
        res.status(StatusCodes.OK).json(createResponse({
          ...
          meta: {
            pagination: {
              ...
              hasNextPage: parseInt(page as string) < result.data.pagination.totalPages,
      ...
      const errorMessage = error instanceof Error ? error.message : WALLET_TRANSACTION_MESSAGES.INTERNAL_SERVER_ERROR;
