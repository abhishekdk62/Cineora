...
export class WalletTransactionRepository implements IWalletTransactionRepository {
  async createWalletTransaction(data: Partial<IWalletTransaction>): Promise<IWalletTransaction> {
    const transaction = new WalletTransaction(data);
    return transaction.save();
  }

  async updateWalletTransactionStatus(transactionId: string, status: string): Promise<IWalletTransaction | null> {
  ...
  async findWalletTransactionById(transactionId: string): Promise<IWalletTransaction | null> {
    return WalletTransaction.findById(transactionId)
      .populate('movieId', 'title')
      .populate('theaterId', 'name');
  }

  async findWalletTransactionByTransactionId(transactionId: string): Promise<IWalletTransaction | null> {
  ...
  async findWalletTransactionsByWalletId(walletId: string): Promise<IWalletTransaction[]> {
  ...
  async findWalletTransactionsByReferenceId(referenceId: string): Promise<IWalletTransaction[]> {
