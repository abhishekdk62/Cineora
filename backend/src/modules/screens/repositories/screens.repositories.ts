...
export class ScreenRepository implements IScreenRepository {
  async createScreen(screenData: CreateScreenDto): Promise<IScreen> {
    try {
      ...
      const processedScreenData = {
        ...
        layout: {
          ...
          advancedLayout: {
            ...
            aisles: {
              vertical: screenData.layout.advancedLayout.aisles?.vertical || [],
              horizontal:
                screenData.layout.advancedLayout.aisles?.horizontal || [],
            },
          },
        },
      };

      const screen = new Screen(processedScreenData);
      ...
      return populatedScreen || savedScreen;
    } catch (error) {
  ...
  async getScreenByIdWithTheaterDetails(
    screenId: string
  ...
  async getScreenByTheaterIdAndName(
    theaterId: string,
    name: string
  ): Promise<IScreen | null> {
    try {
  ...
  private _processUpdateData(updateData: UpdateScreenDto): any {
    ...
    if (processedData.layout?.advancedLayout) {
      if (!processedData.layout.advancedLayout.aisles) {
        ...
        if (!processedData.layout.advancedLayout.aisles.horizontal) {
          processedData.layout.advancedLayout.aisles.horizontal = [];
        }
      }
    }
