...
export type Movie = MovieResponseDto;
...
const MovieSelectionModal: React.FC<MovieSelectionModalProps> = ({
  ...
  const fetchMovies = async () => {
    ...
    try {
      ...
      setMovies((result.data ?? []).filter((movie) => movie.isActive));

    } catch (error) {

      console.error("Error fetching movies:", getApiErrorMessage(error, "Failed to fetch movies"));
  ...
  const handleFinalSelect = (movie: MovieResponseDto) => {
