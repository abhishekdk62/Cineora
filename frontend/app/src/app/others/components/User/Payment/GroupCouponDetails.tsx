...
interface GroupCouponDetailsProps {
...
export const GroupCouponDetails: React.FC<GroupCouponDetailsProps> = ({
  selectedCoupon,
  availableCoupons,
  onSelectCoupon,
  onRemoveCoupon,
      ...
      <div className="pt-6 border-t border-gray-600/30">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className={`${lexendMedium.className} text-white text-lg`}>
              Apply Coupon ({totalSeats} Ticket{totalSeats > 1 ? 's' : ''})
            </h3>
          </div>
        ...
        <div className="mb-4 p-3 bg-purple-500/10 border border-purple-500/30 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <p className={`${lexendSmall.className} text-purple-300 text-sm`}>
              Discount applies to seat prices (₹{totalSeatPrice}) for all {totalSeats} tickets. Taxes & fees calculated on discounted amount.
            </p>
          </div>
        </div>
          ...
          <div className="mb-4">
            <div className="flex gap-2">
              <div className="flex-1 relative">
                <input
                  ...
                  className={`w-full px-4 py-3 bg-white/5 border ${
                    couponError ? 'border-red-500/50' : 'border-purple-500/30'
                  } rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500/50 transition-all`}
                  disabled={isApplying}
                />
              ...
              <button
                ...
                className={`px-6 py-3 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-lg transition-all duration-200 min-w-[100px] disabled:opacity-50 disabled:cursor-not-allowed ${
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`${lexendSmall.className} text-green-300 text-xs`}>
                    {selectedCoupon.discountPercentage}% OFF All Tickets
                  </span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className={`${lexendSmall.className} text-green-300 text-sm font-medium`}>
                  -₹{discount}
                </span>
                <button
                  onClick={onRemoveCoupon}
                  className="text-white hover:text-red-300 transition-colors ml-2 text-sm"
                >
                  Remove
                </button>
              </div>
                ...
                {availableCoupons.map((coupon, index) => (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <div>
                        <h3 className={`${lexendMedium.className} text-white text-base`}>
                          {coupon.name}
                        </h3>
                        <div className="flex items-center gap-2 mt-2">
                          <span className="border border-purple-300 text-white px-2 py-1 rounded text-xs font-bold">
