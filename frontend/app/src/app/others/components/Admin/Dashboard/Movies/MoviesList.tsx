...
const MoviesList: React.FC<MoviesListProps> = ({
  ...
  useEffect(() => {
    const delayedFilter = setTimeout(() => {
      onFiltersChange(localFilters, true) // Reset page when filters change
    }, 500)
  ...
  const availableRatings = Array.from(new Set(allMovies.map(movie => movie.rating))).filter(Boolean)
  const availableLanguages = Array.from(new Set(allMovies.map(movie => movie.language))).filter(Boolean)
  const availableGenres = Array.from(new Set(allMovies.flatMap(movie => movie.genre))).filter(Boolean)
  const releaseYears = Array.from(new Set(allMovies.map(movie => new Date(movie.releaseDate).getFullYear()))).sort((a, b) => b - a)
  ...
  // ... your existing renderMovieCard and renderMovieList functions remain the same ...
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-[#1a1a1a] border border-gray-600 rounded-lg p-4 shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-400">{allMovies.filter(m => m.isActive).length}</p>
            </div>
        ...
        <div className="bg-[#1a1a1a] border border-gray-600 rounded-lg p-4 shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-red-400">{allMovies.filter(m => !m.isActive).length}</p>
            </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="text-sm text-gray-400">
            Showing {movies.length} of {totalItems} results (Page {currentPage} of {totalPages})
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    onChange={(e) => handleFilterChange('releaseYearStart', e.target.value ? parseInt(e.target.value) : undefined)}
                    className="flex-1 px-3 py-2 bg-[#2a2a2a] border border-gray-500 rounded-lg text-white focus:outline-none focus:border-[#e78f03] focus:ring-1 focus:ring-[#e78f03] appearance-none"
                  >
                  ...
                  <select
                    ...
                    onChange={(e) => handleFilterChange('releaseYearEnd', e.target.value ? parseInt(e.target.value) : undefined)}
                    className="flex-1 px-3 py-2 bg-[#2a2a2a] border border-gray-500 rounded-lg text-white focus:outline-none focus:border-[#e78f03] focus:ring-1 focus:ring-[#e78f03] appearance-none"
                  >
      ...
      {movies.length === 0 ? (
          ...
          {isLoading ? (
            ...
            <>
              ...
              <h3 className={`${lexend.className} text-xl text-white mb-2`}>
                {activeFilterCount > 0 ? "No movies match your filters" : emptyMessage}
              </h3>
              <p className={`${lexendSmall.className} text-gray-400`}>
                {activeFilterCount > 0 ? "Try adjusting your filter criteria" : "No movies found"}
              </p>
            </>
          )}
        </div>
        ...
        >
          {movies.map((movie) => (viewMode === "grid" ? renderMovieCard(movie) : renderMovieList(movie)))}
        </div>
      )}
