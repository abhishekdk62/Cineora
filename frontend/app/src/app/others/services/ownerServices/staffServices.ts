...
export const createStaff = async (datas: CreateStaffRequestDto): Promise<CreateStaffResponseDto> => {
  const data = await apiClient.post(OWNER_STAFF.CREATE, datas);
  return data.data;
};

export const verifyTicket = async (datas: string): Promise<CreateStaffResponseDto> => {
  const data = await apiClient.post(OWNER_STAFF.VERIFY_TICKET, { data: datas });
  return data.data;
};

export const getStaffDetails = async (): Promise<GetStaffDetailsResponseDto> => {
