...
export const movieSchema = z.object({
  ...
  duration: z.number().min(1, "Duration must be at least 1 minute").max(500, "Duration too long"),
  rating: z.string().min(1, "Rating is required"),
  genre: z.array(z.string()).min(1, "At least one genre is required"),
  director: z.string().min(1, "Director is required").max(100, "Director name too long"),
...
export const screenSchema = z.object({
  name: z.string().min(2, "Screen name must be at least 2 characters").max(50, "Screen name too long"),
  totalSeats: z.number().min(1, "Total seats must be greater than 0").max(5000, "Maximum 5000 total seats allowed"),
...
export const showtimeCreateSchema = showtimeSchema.merge(z.object({
  dateRange: z.object({
...
export const showtimeEditSchema = showtimeSchema.merge(z.object({
...
export const theaterSchema = z.object({
  name: z.string().min(1, "Theater name is required").max(100, "Theater name is too long"),
